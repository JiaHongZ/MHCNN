        ...
        for k in range(20):
            ...
            for yy in range(2):
                for xx in range(2):
                    ...
                    use_cuda = 1
                    # Inoisy_crop = torch.Tensor(Inoisy_crop)
                    noisy = torch.from_numpy(Inoisy)
                    if len(noisy.shape) > 2:
                        noisy = noisy.reshape(1, noisy.shape[2], noisy.shape[0], noisy.shape[1])
                    else:
                        noisy = noisy.reshape(1, 1, noisy.shape[0], noisy.shape[1])
                    # if use_cuda:
                    #     noisy = noisy.cuda()
                    ...
                    Idenoised_crop = np.transpose(denoised, [1, 2, 0])
